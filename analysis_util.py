...
def get_strategies_plot_data(df, v1_only, v2_only, v1v2, v2v1, start, end, step, last_delta):
    data = []
    for delta in range(start, end, step):
        df_delta = df[(df['chunk_size_delta'] > last_delta) & (df['chunk_size_delta'] <= delta)]
        if len(df_delta) > 0:
            v1_c = v1_only[(v1_only['chunk_size_delta'] > last_delta) & (v1_only['chunk_size_delta'] <= delta)]
            v2_c = v2_only[(v2_only['chunk_size_delta'] > last_delta) & (v2_only['chunk_size_delta'] <= delta)]
            v1v2_c = v1v2[(v1v2['chunk_size_delta'] > last_delta) & (v1v2['chunk_size_delta'] <= delta)]
            v2v1_c = v2v1[(v2v1['chunk_size_delta'] > last_delta) & (v2v1['chunk_size_delta'] <= delta)]
...
def get_strategies_plot_df(df, v1_only, v2_only, v1v2, v2v1):
    delta_max = int(df['chunk_size_delta'].max())
    delta_min = int(df['chunk_size_delta'].min())
    data = []
    data.extend(get_strategies_plot_data(df, v1_only, v2_only, v1v2, v2v1, delta_min, 0, 4, delta_min-1))
    data.extend(get_strategies_plot_data(df, v1_only, v2_only, v1v2, v2v1, 0, 1, 1, -1))
    data.extend(get_strategies_plot_data(df, v1_only, v2_only, v1v2, v2v1, 1, delta_max, 4, 0))
